   ...
   if (json["fittingModel"].HasMember("pulls")) {
      for (const auto& pull : json["fittingModel"]["pulls"].GetObject()) {
         ...
         // initialize and add exponential pulls
            ...
            fitter->AddModel(mod);
         } else if (strcmp(pull.value["type"].GetString(), "ratio")==0) {
           BXModelPullRatio* mod = new BXModelPullRatio(pull.name.GetString());
           mod->SetNumPar(pull.value["numerator"].GetString());
           mod->SetDenPar(pull.value["denumerator"].GetString());
           mod->SetGaussPar(pull.value["centroid"].GetDouble(),
               pull.value["sigma"].GetDouble());
           fitter->AddModel(mod);
         } else if (strcmp(pull.value["type"].GetString(), "sum") == 0) {
           BXModelPullSum* mod = new BXModelPullSum(pull.name.GetString());
           mod->SetPar1(pull.value["par1"].GetString());
           mod->SetPar2(pull.value["par2"].GetString());
           mod->SetGaussPar(pull.value["centroid"].GetDouble(),
               pull.value["sigma"].GetDouble());
           fitter->AddModel(mod);
         } 
...
TCanvas* GetCanvasProfiles (const rapidjson::Document& json, MSMinimizer* fitter, 
   ...
   int parIndex = 0;
   for ( auto it : *fitter->GetParameterMap()) {
     if (strcmp(it.second->GetName().c_str(), "global.CNO") == 0 ||
         strcmp(it.second->GetName().c_str(), "global.Bi210") == 0) { // HACK TO DRAW NPL FOR CNO AND BI210 ONLY
